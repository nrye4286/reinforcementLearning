...
class DQNSkiing(gym.Wrapper):
    def __init__(self, render_mode='rgb_array_list', repeat=4, device='cpu'):
        env = gym.make('ALE/Skiing-v5', render_mode=render_mode, frameskip=4, repeat_action_probability=0.0)
        super(DQNSkiing, self).__init__(env)
    ...
    def step(self, action):
        ...
        for i in range(self.repeat):
            ...
            if len(self.frame_buffer2) == 0:
                self.frame_buffer2.append(observation[5:14,66:-78].copy())
            self.frame_buffer2.append(observation[5:14,66:-78].copy())
            
            if reward < -2:
                reward = -2
                
            if (self.frame_buffer2[-1] == self.frame_buffer2[-2]).mean() != 1:
                reward += 100
                
            observation_copy = observation.copy()
            observation[observation[:,:] == np.stack((observation_copy.mean(2),observation_copy.mean(2),observation_copy.mean(2)),2)] = 0
            total_reward += reward
        ...
        max_frame = np.array(self.frame_buffer[-1*self.repeat:])
        max_frame[:-1] = max_frame[:-1] * 0.5
        max_frame = np.max(max_frame, axis=0)
    ...
    def process_observation(self, observation):  
        img = Image.fromarray(observation[30:-30])
...
environment = DQNSkiing(device=device)
model = AtariNet(nb_action=3).to(device)
